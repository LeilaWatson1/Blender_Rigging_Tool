...
def create_revolver_template(context, rig_name, prefix="", grip_socket=True, flash_socket=True):
    ...
    create_bone(context, rig_name, f"{p}hammer",  True, True, parent_bone_name=f"{p}local",  ctrl_radius=0.05, ctrl_axis='X', bone_head=(-0.15, 0.0, 0.35), bone_tail=(-0.05, 0.0, 0.35), ctrl_offset=(0.0, 0.0, 0.0), widget_type='arc_arrow',                          ctrl_color=(0.0, 0.0, 0.8))
    create_cylinder_part(context, rig_name, parent_bone_name=f"{p}local", base_name=f"{p}cylinder")
    if grip_socket or flash_socket:
        armatures_visible(rig_name)
    if grip_socket:
