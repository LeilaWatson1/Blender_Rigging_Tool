...
def create_cylinder_part(context, rig_name, parent_bone_name="root", base_name="cylinder", front_axis='X'):
    ...
    follow_bone.head = ax((0.0, 0.1, 0.2))
    follow_bone.tail = ax((0.0, 0.2, 0.2))
    ...
    copy_rot.use_x = (front_axis == 'X')
    copy_rot.use_y = (front_axis == 'Y')
    ...
    add_template(context, rig_name, f"follow_{cyl_name}", parent_bone=latch_name,
                 bone_head=ax((0.0, 0.1, 0.2)), bone_tail=ax((0.0, 0.2, 0.2)))
